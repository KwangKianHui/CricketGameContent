...
class FixturesRemoteService {
  FixturesRemoteService();

  /// Retrieve list of series data through cricket game API.
  /// Converts json to list of SeriesDTO and returns List<SeriesDTO>.
  Future<List<FixtureDTO>> getNext50FixturesDataList() async {
    final response = await http.get(
        Uri.parse("https://cricket-live-data.p.rapidapi.com/fixtures"),
  ...
  Future<List<FixtureDTO>> getFixtureListByDate(String date) async {
    final response = await http.get(
        Uri.parse(
            "https://cricket-live-data.p.rapidapi.com/fixtures-by-date/$date"),
        headers: {"X-RapidAPI-Key": API_TOKEN, 'X-RapidAPI-Host': API_HOST});
